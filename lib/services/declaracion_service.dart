...
class DeclaracionService {
  ...
  Future<List<Declaracion>> obtenerDeclaracionesPorEmpresa(int empresaId) async {
    final db = await _databaseService.database;
    final List<Map<String, dynamic>> maps = await db.query(
      'Declaraciones',
      where: 'empresa_id = ?',
  ...
  Future<List<Declaracion>> obtenerDeclaracionesPorPeriodo(DateTime periodo) async {
    final db = await _databaseService.database;
  ...
  Future<String> generarPDF(Declaracion declaracion) async {
    ...
    final fileName = 'declaracion_${declaracion.tipo.name}_${DateFormat('yyyy_MM').format(declaracion.periodo)}_${declaracion.id}.pdf';
  ...
  Future<void> previsualizarPDF(Declaracion declaracion) async {
    ...
    await Printing.layoutPdf(
      ...
      name: 'Declaración ${declaracion.tipo.name.toUpperCase()} - ${DateFormat('MMMM yyyy', 'es').format(declaracion.periodo)}',
    );
  }
  ...
  Future<void> compartirPDF(Declaracion declaracion) async {
    ...
    if (declaracion.archivoPdf != null && File(declaracion.archivoPdf!).existsSync()) {
      pdfPath = declaracion.archivoPdf!;
    } else {
      pdfPath = await generarPDF(declaracion);
    }
    
    final fileName = 'declaracion_${declaracion.tipo.name}_${DateFormat('yyyy_MM').format(declaracion.periodo)}.pdf';
    
    await Share.shareXFiles(
      [XFile(pdfPath)],
      text: 'Declaración ${declaracion.tipo.name.toUpperCase()} - ${DateFormat('MMMM yyyy', 'es').format(declaracion.periodo)}',
      subject: fileName,
    );
  }
  ...
  Declaracion _mapToDeclaracion(Map<String, dynamic> map) {
    return Declaracion(
      ...
      datos: map['datos_json'] != null 
          ? jsonDecode(map['datos_json'])
          : {},
  ...
  Future<RegimenTributario?> _obtenerRegimen(int regimenId) async {
    ...
    return regimenes.firstWhere((r) => r.id == regimenId, orElse: () => regimenes.first);
  }
  
  // ===== CONSTRUCCIÓN DE PDF =====
  
  pw.Widget _buildPDFHeader(Declaracion declaracion, Empresa? empresa, RegimenTributario? regimen) {
    return pw.Container(
  ...
  pw.Widget _buildPDFContent(Declaracion declaracion, Empresa? empresa, RegimenTributario? regimen) {
    return pw.Column(
      crossAxisAlignment: pw.CrossAxisAlignment.start,
      children: [
        pw.Text(
        ...
        pw.Container(
          ...
          child: pw.Column(
            children: [
              _buildPDFInfoRow('Tipo de Declaración:', declaracion.tipo.name.toUpperCase()),
              _buildPDFInfoRow('Estado:', _getEstadoDescripcion(declaracion.estado)),
              _buildPDFInfoRow('Monto a Pagar:', 'S/ ${declaracion.monto.toStringAsFixed(2)}'),
              _buildPDFInfoRow('Fecha de Creación:', DateFormat('dd/MM/yyyy HH:mm').format(declaracion.fechaCreacion)),
              if (declaracion.fechaPresentacion != null)
                _buildPDFInfoRow('Fecha de Presentación:', DateFormat('dd/MM/yyyy HH:mm').format(declaracion.fechaPresentacion!)),
              if (declaracion.numeroFormulario != null)
                _buildPDFInfoRow('Número de Formulario:', declaracion.numeroFormulario!),
          ...
          pw.Container(
            ...
            child: pw.Column(
              children: declaracion.datos.entries.map((entry) {
                return _buildPDFInfoRow('${entry.key}:', entry.value.toString());
              }).toList(),
            ),
          ),
        ],
  ...
  Future<Map<String, dynamic>> obtenerResumenAnual(int anio) async {
    ...
    final declaraciones = List.generate(maps.length, (i) => _mapToDeclaracion(maps[i]));
